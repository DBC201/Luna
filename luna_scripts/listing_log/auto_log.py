...
if __name__ == '__main__':
    ...
    process_output_folder = os.path.join(ROOT, "outputs")
    symbols = {}

    if not os.path.exists(process_output_folder):
        ...
        if listing_time is not None and time.time() + 60 >= listing_time:
            for symbol in symbols:
                for quote in symbols[symbol]:
                    process_output = os.path.join(process_output_folder, symbol+quote+"_binance_log.txt")
                    subprocess.Popen(
